        ...
        match action {
            ...
            "save" => {
                let key = input
                    .get("key")
                    .and_then(Value::as_str)
                    .ok_or_else(|| ToolError::new("missing key".to_string()))?;
                let content = input
                    .get("content")
                    .and_then(Value::as_str)
...
fn validate_key(key: &str) -> Result<(), ToolError> {
    if key.len() > 64 {
        return Err(ToolError::new("key too long".to_string()));
    }
    if key.starts_with("system_") || key.starts_with("internal_") {
        return Err(ToolError::new("key prefix is reserved".to_string()));
    }
    let mut chars = key.chars();
    let first = chars
        .next()
        .ok_or_else(|| ToolError::new("key required".to_string()))?;
